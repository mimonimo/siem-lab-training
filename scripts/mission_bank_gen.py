...
# --- per-question primer: 정답을 알려주지 않고 '개념/무엇을 볼지'만 안내(초보자용) --- #
PRIMERS = {
 1:  "브루트포스는 비밀번호를 반복 대입하는 공격입니다. 인증 로그에서 '실패(Failed)가 여러 번 이어진 뒤 성공(Accepted)'한 지점이 최초 침투 시각입니다.",
 2:  "공격이 어디서 왔는지는 인증 로그의 출발지 IP로 확인합니다. 여러 계정을 연달아 시도한 동일 IP에 주목하세요.",
 3:  "웹셸은 웹서버에 올려두고 URL로 명령을 실행시키는 악성 스크립트입니다. 웹 루트(/var/www/html)에 새로 생긴 스크립트 파일을 찾습니다.",
 4:  "웹셸은 URL의 cmd= 파라미터로 명령을 전달받습니다. access.log에서 cmd= 가 처음 등장한 요청을 봅니다.",
 5:  "공격자는 추가 악성코드를 wget으로 내려받습니다. 다운로드 명령에서 저장 위치(경로)를 확인하세요.",
 6:  "지속성(persistence)은 재부팅·재접속에도 공격이 살아남게 하는 수법입니다. cron이 반복 실행하도록 등록된 대상 명령을 봅니다.",
 7:  "공격자는 시스템 명령처럼 위장한 파일을 심어둡니다. auditd의 변조 기록(bin_tamper)에서 대상 경로를 찾습니다.",
 8:  "실행파일 안의 사람이 읽을 수 있는 문자열(strings)에는 공격 흔적이 남습니다. 로그 원문에서 관련 키워드를 찾아보세요.",
 9:  "권한 상승은 일반 사용자가 root 권한을 얻는 것입니다. 공격자는 재접속에도 권한이 유지되도록 sudo 관련 설정을 몰래 바꿔두곤 합니다. 언제·어떤 계정이 권한 관련 변경을 일으켰는지 인증·감사(auditd) 로그에서 추적해 보세요.",
 10: "서로 다른 사건이 동일 공격자인지는 출발지 IP·사용 계정이 같은지로 판단합니다.",
 11: "정보 유출 직전, 공격자는 빼낼 파일을 tar로 하나로 묶습니다. 압축 결과 파일(.tgz)의 경로를 봅니다.",
 12: "유출(exfiltration)은 데이터를 외부로 전송하는 것입니다. 업로드(curl) 명령의 목적지 IP를 확인합니다.",
 13: "침투→권한상승→유출로 이어지는 종합 문제입니다. 각 로그 줄의 실제 시각을 읽어 시간순으로 배열하세요.",
 14: "정상 관리자의 야간 작업은 공격처럼 보여도 정상일 수 있습니다. 계정·출발지·작업 목적이 정상 업무와 맞는지로 판단합니다.",
 15: "반복 로그인 실패 후 계정이 잠겼다면 방어가 작동한 것입니다. 성공한 인증이 있는지가 핵심입니다.",
 16: "짧은 시간의 다수 404 + 스캐너 User-Agent는 정찰(스캔) 신호입니다. 실제 침해(성공·후속행위)로 이어졌는지가 판단 기준입니다.",
 17: "auditd는 프로세스 실행마다 의미를 담은 key= 태그를 붙입니다. 웹셸이 실행한 명령에 붙은 키 이름을 봅니다.",
 18: "cron 지속성 등록 행위에도 auditd key= 가 붙습니다. 해당 태그 값을 찾으세요.",
 19: "알림이 떴다고 모두 대응하는 것이 아닙니다. 실제 침해로 이어진 IP만 골라내는 판단이 핵심입니다.",
 20: "야간에 등록된 두 cron 중 어느 것이 악성인지, 계정과 실행 대상 경로를 비교해 구분합니다.",
 21: "웹셸로 실행된 여러 명령을 관측된 시간 순서대로 나열합니다.",
 22: "auditd는 실행된 프로세스 이름을 comm= 필드에 기록합니다. 웹셸 명령들이 실제 실행됐는지 교차 확인하는 문제입니다.",
 23: "압축(tar) 명령에 포함된 대상 디렉터리·파일을 봅니다. 무엇을 빼내려 했는지 드러납니다.",
 24: "공격 지속 시간은 최초 침투(A1)와 유출 완료(E2)의 '실제 시각' 차이로 계산합니다. (Time 컬럼이 아니라 로그 본문 시각)",
 25: "Wazuh 커스텀 룰 100101이 무엇을 탐지하는지 이해하고, 대시보드에서 그 알림 건수를 셉니다.",
 26: "한 IP로 필터하면 그 공격자가 남긴 흔적이 여러 로그 파일(location)에 걸쳐 나타납니다. 어떤 소스들인지 봅니다.",
 27: "계정 잠금은 몇 번 실패에 발동했는지(pam_faillock 임계치)가 핵심입니다.",
 28: "웹셸 파일이 '배치된 시각'과 '첫 명령이 실행된 시각'의 선후 관계를 따집니다. 배치가 먼저여야 자연스럽습니다.",
 29: "정상 cron과 악성 cron의 파일 경로를 각각 확인해 구분하는 문제입니다.",
 30: "설정 파일에 심어진 sudo 백도어는 '비밀번호 없이 root 실행'을 허용하는 것이 핵심입니다. 이런 설정 문구는 대문자·콜론 등이 섞여 정확히 일치검색이 어려우니, full_log 필드에서 부분일치(와일드카드) 검색으로 좁혀가는 방법이 유용합니다. 어떤 키워드가 '무인증 sudo'를 뜻할지 떠올려 검색어를 만들어 보세요.",
 31: "알림은 떴지만 실제 대응이 불필요한 사례(정찰·정상업무·방어성공)를 모두 찾아 이유를 설명합니다.",
 32: "라이브 데모 버튼을 누른 뒤 Wazuh에서 새로 발생하는 탐지 룰(rule.id)을 확인하는 문제입니다.",
 33: "웹 루트에 파일이 생성된 사건을 기록한 auditd key= 값을 찾습니다.",
 34: "두 공격 IP가 각각 어떤 역할(공격 본체 / 다운로드·유출 서버)인지 로그 근거로 설명합니다.",
 35: "웹셸을 웹 루트로 옮긴 sudo 복사(cp) 명령의 '원본 경로 → 대상 경로'를 봅니다.",
 36: "각 사건을 공격 전술 단계(초기 접근·지속성·유출)에 짝짓는 MITRE ATT&CK 관점의 종합 문제입니다.",
}
...
"""]
    ...
    for i, q in enumerate(ordered, start=1):
        ...
        spec["questions"].append({
            ...
            "primer": PRIMERS.get(i, ""),     # free conceptual hint (no answer) shown on card
